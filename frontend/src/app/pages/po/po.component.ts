...
@Component({
    ...
    <section class="page-shell">
      <div class="page-header">
        <div>
          ...
          <p>All purchase orders for the logged in vendor.</p>
        </div>
      ...
      <mat-card class="data-card">
        <div class="card-toolbar">
          <div>
            <mat-card-title>{{ poList.length }} purchase orders</mat-card-title>
            <mat-card-subtitle>Order type, document date, vendor reference, and currency</mat-card-subtitle>
          </div>
          <div class="table-actions" *ngIf="poList.length">
            <mat-form-field appearance="outline" class="search-field">
              ...
              <input
                ...
                placeholder="PO, type, vendor, currency, date"
              />
              <button
                *ngIf="searchTerm"
                mat-icon-button
        ...
        <div class="table-wrap" *ngIf="poList.length; else emptyState">
          <table mat-table [dataSource]="dataSource" class="po-table">
  ...
  styles: [
    `
      ...
      .po-table{width:100%;min-width:760px;background:#fff}
      .po-table th{background:#f8f3ef;color:#5e5154;font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em}
      .po-table td{color:#2f292b}
      .po-table th,.po-table td{padding:0 1.15rem;border-bottom:1px solid rgba(87,60,60,.1)}
      .po-table tr.mat-mdc-row{height:62px}
      .po-table tr.mat-mdc-row:hover{background:#fff8f7}
...
export class PoComponent implements OnInit {
  ...
  displayedColumns = ['po', 'type', 'date', 'vendor', 'currency'];
  ...
  async ngOnInit() {
    try {
      ...
      this.dataSource.data = this.poList;
    } catch {
      this.poList = [];
