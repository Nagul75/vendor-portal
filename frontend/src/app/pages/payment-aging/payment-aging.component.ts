...
@Component({
    ...
    <section class="page-shell">
      ...
      <mat-card class="data-card">
        <div class="card-toolbar">
          ...
          <div class="table-actions" *ngIf="paymentList.length">
            <mat-form-field appearance="outline" class="search-field">
              <mat-label>Search payment aging</mat-label>
              ...
              <input
                ...
                placeholder="Document, year, amount, status, aging"
              />
              <button
                *ngIf="searchTerm"
                mat-icon-button
            ...
            <mat-chip-set aria-label="Payment aging record count">
        ...
        <div class="table-wrap" *ngIf="paymentList.length; else emptyState">
          <table mat-table [dataSource]="dataSource" class="payment-table">
            ...
            <ng-container matColumnDef="year">
              <th mat-header-cell *matHeaderCellDef>Invoice Year</th>
              <td mat-cell *matCellDef="let item">{{ item.FiscalYear || '-' }}</td>
            </ng-container>
            ...
            <ng-container matColumnDef="aging">
              ...
              <td mat-cell *matCellDef="let item">
                <span class="aging-code">{{ item.AgingDays || '-' }}</span>
              </td>
            </ng-container>
            ...
            <tr class="mat-row no-results-row" *matNoDataRow>
              <td class="mat-cell" [attr.colspan]="displayedColumns.length">
                No payment aging records match your search.
              </td>
            </tr>
          </table>
        </div>
        <mat-paginator
          *ngIf="paymentList.length"
          ...
          aria-label="Payment aging pagination"
        ></mat-paginator>
      </mat-card>
  ...
  styles: [
    `
      ...
      .payment-table{width:100%;min-width:860px;background:#fff}
      .payment-table th{background:#f8f3ef;color:#5e5154;font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em}
      .payment-table td{color:#2f292b}
      .payment-table th,.payment-table td{padding:0 1.15rem;border-bottom:1px solid rgba(87,60,60,.1)}
      .payment-table tr.mat-mdc-row{height:62px}
      .payment-table tr.mat-mdc-row:hover{background:#fff8f7}
      ...
      .status-pill{display:inline-flex;align-items:center;min-height:28px;padding:.2rem .65rem;border-radius:8px;background:#f5e8e7;color:#760407;font-weight:800}
      .aging-code{display:inline-flex;align-items:center;justify-content:center;min-width:48px;min-height:28px;padding:.15rem .5rem;border-radius:8px;background:#fff3e8;color:#8a4300;font-weight:800}
...
export class PaymentAgingComponent implements OnInit {
  ...
  displayedColumns = ['doc', 'year', 'dueDate', 'amount', 'status', 'aging'];
  paymentList: PaymentAging[] = [];
  dataSource = new MatTableDataSource<PaymentAging>([]);
  searchTerm = '';
