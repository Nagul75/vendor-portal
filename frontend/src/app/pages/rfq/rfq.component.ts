    ...
    <section class="page-shell">
      <div class="page-header">
        <div>
          ...
          <p>All RFQs for the logged in vendor.</p>
        </div>
      ...
      <mat-card class="data-card">
        <div class="card-toolbar">
          <div>
            <mat-card-title>{{ rfqList.length }} RFQs</mat-card-title>
            <mat-card-subtitle>Quotation request type, document date, vendor, and currency</mat-card-subtitle>
          </div>
          <div class="table-actions" *ngIf="rfqList.length">
            <mat-form-field appearance="outline" class="search-field">
              ...
              <input
                ...
                placeholder="RFQ, type, vendor, currency, date"
              />
              <button
                *ngIf="searchTerm"
                mat-icon-button
            ...
            <mat-chip-set aria-label="RFQ record count">
        ...
        <div class="table-wrap" *ngIf="rfqList.length; else emptyState">
          <table mat-table [dataSource]="dataSource" class="rfq-table">
            <ng-container matColumnDef="rfq">
              <th mat-header-cell *matHeaderCellDef>RFQ</th>
              <td mat-cell *matCellDef="let item">
                <span class="primary-cell">{{ item.Ebeln || '-' }}</span>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="type">
              ...
              <td mat-cell *matCellDef="let item">
                <span class="type-pill">{{ item.Bsart || '-' }}</span>
              </td>
            </ng-container>
            ...
            <ng-container matColumnDef="vendor">
              ...
              <td mat-cell *matCellDef="let item">{{ item.Lifnr || '-' }}</td>
            </ng-container>

            <ng-container matColumnDef="currency">
              <th mat-header-cell *matHeaderCellDef>Currency</th>
              <td mat-cell *matCellDef="let item">
                <span class="currency-code">{{ item.Waers || '-' }}</span>
              </td>
            </ng-container>
      ...
      <ng-template #emptyState>
        <p class="empty-message">No RFQs found.</p>
      </ng-template>
    </section>
  `,
  styles: [
    `
      ...
      .rfq-table{width:100%;min-width:760px;background:#fff}
      .rfq-table th{background:#eff8f6;color:#4b5d59;font-size:.75rem;font-weight:800;text-transform:uppercase;letter-spacing:.06em}
      .rfq-table td{color:#2f292b}
      .rfq-table th,.rfq-table td{padding:0 1.15rem;border-bottom:1px solid rgba(87,60,60,.1)}
      .rfq-table tr.mat-mdc-row{height:62px}
      .rfq-table tr.mat-mdc-row:hover{background:#f7fcfb}
      .primary-cell{font-weight:800;color:#21191b}
      .type-pill{display:inline-flex;align-items:center;min-height:28px;padding:.2rem .6rem;border-radius:8px;background:#e5f3f1;color:#0f766e;font-weight:800}
      .date-cell{display:inline-flex;align-items:center;gap:.4rem;color:#3a3033;font-weight:650}
      .date-cell mat-icon{width:18px;height:18px;font-size:18px;color:#0f766e}
      .currency-code{display:inline-flex;align-items:center;justify-content:center;min-width:48px;min-height:28px;padding:.15rem .5rem;border-radius:8px;background:#f1f5f4;color:#0f766e;font-weight:800}
...
export class RfqComponent implements OnInit {
  ...
  displayedColumns = ['rfq', 'type', 'date', 'vendor', 'currency'];
  rfqList: Rfq[] = [];
  ...
  async ngOnInit() {
    try {
      const response = await this.api.getRfqs();
      this.rfqList = response.data || [];
      this.dataSource.data = this.rfqList;
    } catch {
      this.rfqList = [];
